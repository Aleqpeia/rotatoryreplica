...
def _format_training_set(systems, phases=['ordered', 'fluid']):

    # Check the phases format
    if not core._is_list_of(phases, type='string', check_array=True, recursive=False):
        core._error_input_type('Phases', 'list of '+str(str))
...
def _get_models_from_source(models):
    ...
    if core._is_file_is(models, extensions=['.lpm'], exist=True, no_extension=False):
    ...
    elif core._is_dict(models):
        return models, None

    else:
        core._error_input_type("Models","model file or model dictionary")
...
def trainModel(coordinates, distances, states, validationSize=0.20, seed=7, nSplits = 10):
    ...
    # Check the inputs
    if not core._is_float(validationSize, strict=True):
        core._error_input_type('First frame',str(float))
    elif validationSize < 0.01 or validationSize >= 0.33:
        core._error_out_of_range(validationSize, 'Validation size', 0.01, 0.33)

    if not core._is_int(seed):
        core._error_input_type('Seed',str(int))

    if not core._is_int(nSplits):
        core._error_input_type('Number repetitions',str(int))
    elif nSplits <= 0:
        core._error_out_of_range(nSplits, 'Number repetitions', 1, "inf")

    # Check the validation subset size
    core._error_training_size(coordinates.shape[0] / np.unique(states).shape[0], validationSize)
